...
//				internalPacketPool.ReleasePointer( internalPacket );
	...
	while ( internalPacket )
	{
		...
		for (i=0; i < messageHandlerList.Size(); i++)
			res = res && messageHandlerList[i]->OnInternalPacket(internalPacket, receivePacketCount, playerId, (RakNetTime)(time/(RakNetTimeNS)1000), false);
		if (!res) {
		...
		}
		{
			...
			{
				//receivedPacketsBaseIndex = internalPacket->messageNumber;
			...
			if ( internalPacket->reliability == RELIABLE_ORDERED )
			{
				...
				//printf("trying %i\n", internalPacket->orderingIndex - waitingForOrderedPacketReadIndex[ internalPacket->orderingChannel ] );
				//printf("waitingForOrderedPacketReadIndex = %d; internalPacket->orderingIndex = %d\n", waitingForOrderedPacketReadIndex[ internalPacket->orderingChannel ], internalPacket->orderingIndex);
