...
public:
	...
	virtual bool OnInternalPacket(InternalPacket *internalPacket, unsigned frameNumber, PlayerID remoteSystemID, RakNetTime time, bool isSend);

	virtual bool OnSendImmediate(const char *data, const unsigned bitsUsed);
	virtual void OnRpcReceive(unsigned char rpc_id, RPCParameters* params);
};
