...
bool BitStream::ReadBit(void) {
    unsigned char d = data[readOffset >> 3];
    return (bool)(d & (0x80 >> (readOffset++ & 7)));
}
...
void BitStream::AddBitsAndReallocate(const int numberOfBitsToWrite) {
    ...
    if (numberOfBitsToWrite + numberOfBitsUsed > 0 && ((numberOfBitsAllocated - 1) >> 3) < (
            (newNumberOfBitsAllocated - 1) >> 3)) {
...
void BitStream::AssertCopyData(void) {
    if (copyData == false) {
        ...
        if (numberOfBitsAllocated > 0) {
            auto newdata = (unsigned char*)malloc(BITS_TO_BYTES(numberOfBitsAllocated));
